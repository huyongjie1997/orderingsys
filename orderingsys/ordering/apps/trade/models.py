...
class ShoppingCart(models.Model):
    ...
    food = models.ForeignKey(Food,on_delete=models.CASCADE, verbose_name=u"菜品")
    ...
    class Meta:
        ...
        unique_together = ("user", "food")# 一个商品不应该在购物车中重复

    def __str__(self):
        return ("%s,%d")%(self.food.name,self.nums)
...
class OrderInfo(models.Model):
    ...
    orderinfo_status = models.IntegerField(default="-8", verbose_name="订单状态")
    order_mount = models.FloatField(default=0.0, verbose_name="订单金额")
    pay_time = models.DateTimeField(null=True, blank=True, verbose_name="支付时间")
    comment_status = models.BooleanField(default=False, verbose_name="是否评论")
    ...
    def __str__(self):
        return "(%d,%s)"%(self.id,str(self.order_sn))
...
class OrderFoods(models.Model):
    ...
    food_num = models.IntegerField(default=0, verbose_name="菜品数量")
    ...
    def __str__(self):
        return ('%s,%d')%(str(self.order.order_sn),self.food.id)